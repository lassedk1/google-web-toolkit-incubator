...
class DOMImplSafari extends DOMImplStandard {
  public Element createTD() {
    ...
    setElementPropertyInt(td, "colSpan", 1);
    setElementPropertyInt(td, "rowSpan", 1);
    return td;
  }
  
  public Element createTH() {
    Element th = super.createTH();
    setElementPropertyInt(th, "colSpan", 1);
    setElementPropertyInt(th, "rowSpan", 1);
    return th;
  }
   ...
   *
   * The cellIndex property is not defined in Safari, so we
   * must calculate the cell index manually.
   *
   * @param td the cell element
   * @return the cell index
   */
  public int getCellIndex(Element td) {
    return getChildIndex(getParent(td), td);
  }
