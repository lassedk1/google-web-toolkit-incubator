 ...
 * </ul>
 */
public abstract class AbstractScrollTable extends ComplexPanel implements ResizableWidget, HasScrollHandlers {
	/**
	...
	private static class MouseResizeWorker {
		...
		/**
		 * A timer used to resize the columns. As long as the timer is active,
		 * it will poll for the new row size and resize the columns.
		 */
		...
		/**
		 * The cells that will be sacrificed so the current cells can be
		 * resized.
		 */
		...
		/**
		 * Resize the column on a mouse event. This method also marks the client
		 * as busy so we do not try to change the size repeatedly.
		 * 
		 * @param event
		 *            the mouse event
		 */
		...
		public boolean setCurrentCell(Event event) {
			...
			if (cell != curCell) {
				...
				if (curCell != null) {
					...
					// Check for resizable columns within one of the cells in
					// the colspan
		...
		/**
		 * Start resizing the current cell when the user clicks on the right
		 * edge of the cell.
		 * 
		 * @param event
		 *            the mouse event
		...
		/**
		 * Helper method that actually sets the column size. This method is
	 ...
	 * isn't updated if the mouse is hovering over an element DOM object when
	 * its cursor style is changed.
	 */
	 ...
	 * Resources used.
	 */
	public interface ScrollTableStyle extends ImmutableResourceBundle {
		/**
		 * The css file.
		 */
		@Resource("com/google/gwt/gen2/widgetbase/public/ScrollTable.css")
		Css css();
		 ...
		 */
		@Resource("scrollTableFillWidth.gif")
		 ...
		 * @return an image resource of this image
		 */
		@Resource("scrollTableAscending.gif")
	...
	protected static class DefatulScrollTableResources implements ScrollTableResources {
	 ...
	 * 
	 * <ul>
	 * <li>UNCONSTRAINED - Columns shrink and expand independently of each other
	 * </li>
	 * <li>FLOW - As one column expands or shrinks, the columns to the right
	 * will do the opposite, trying to maintain the same size. The user can
	 * still expand the grid if there is no more space to take from the columns
	 * on the right.</li>
	 * <li>FIXED_WIDTH - As one column expands or shrinks, the columns to the
	 * right will do the opposite, trying to maintain the same size. The width
	 * of the grid will remain constant, ignoring column resizing that would
	 * result in the grid growing in size.</li>
	...
	public AbstractScrollTable(FixedWidthGrid dataTable, FixedWidthFlexTable headerTable) {
		this(dataTable, headerTable, new DefatulScrollTableResources());
	}
	 ...
	 *            the header table
	 * @param images
	 *            the images to use in the table
	 */
	public AbstractScrollTable(FixedWidthGrid dataTable, FixedWidthFlexTable headerTable, ScrollTableResources resources) {
		super();
		...
		setElement(mainElem);
		setStylePrimaryName(css.defaultStyleName());
		...
		dataWrapper = createWrapper(css.dataWrapper());
		...
		fillWidthImage.setUrlAndVisibleRect(imageResource.getURL(), imageResource.getLeft(), imageResource.getTop(), imageResource.getWidth(), imageResource
				.getHeight());
	 ...
	 * 
	 * The {@link AbstractScrollTable} must be visible on the page for this
	 * method to work.
	 */
		...
		case Event.ONMOUSEUP:
			...
			if (resizeWorker.isResizing()) {
				...
				if (cellElem != null) {
					...
					int column = headerTable.getColumnIndex(row, cell) - getHeaderOffset();
					if (column >= 0 && isColumnSortable(column) && isHeaderRowSortable(row)) {
	...
	/**
	 * Redraw the table. This is a relatively cheap operation and should be
	 * called after modifying the header or footer sections.
	 */
	public void redraw() {
		if (!isAttached()) {
			return;
	 ...
	 *            true to enable column filtering
	 */
	...
	/**
	 * Set the footer table that appears under the data table. If set to null,
	 * the footer table will not be shown.
	 * 
	 * @param footerTable
	 *            the table to use in the footer
	 */
	public void setFooterTable(FixedWidthFlexTable footerTable) {
		...
		if (footerTable != null) {
			...
			prepareTable(footerTable, css.footerTable());
			if (dataTable.getSelectionPolicy().hasInputColumn()) {
			...
			if (footerWrapper == null) {
				footerWrapper = createWrapper(css.footerWrapper());
			...
			adoptTable(footerTable, footerWrapper, getElement().getChildNodes().getLength());
		}
	 ...
	 * @param sortable
	 *            true to enable sorting for this column, false to disable
	 */
	 ...
	 * 
	 * @param tdElem
	 *            the cell in the header table, or null to remove it
	 * @param ascending
	 *            true to apply the ascending indicator, false for descending
	 */
	protected void applySortedColumnIndicator(Element tdElem, boolean ascending) {
		...
		sortedColumnIndicator.setUrlAndVisibleRect(imageResource.getURL(), imageResource.getLeft(), imageResource.getTop(), imageResource.getWidth(),
	 ...
	 * 
	 * @param cssName
	 *            the style name added to the base name
	...
	protected void maybeFillWidth() {
		if (!fillWidthPending && isAttached() && resizePolicy == ResizePolicy.FILL_WIDTH) {
	...
	/**
	 * Fixes the table heights so the header is visible and the data takes up
	...
	protected void resizeTablesVerticallyNow() {
		...
		DOM.setStyleAttribute(dataWrapper, "height", Math.max(totalHeight - headerHeight - footerHeight, 0) + "px");
	 ...
	 * 
	 * @param baseHeader
	 *            If true, use the header as the alignment source
	 */
	...
	/**
	 * Get the offset between the data and header and footer tables. An offset
	 * of one means that the header and footer table indexes are one greater
	 * than the data table indexes, probably because the data table contains a
	 * checkbox column.
	 * 
	 * @return the offset
	 */
