...
import com.google.gwt.widgetideas.table.client.filter.ColumnFilter;
import com.google.gwt.widgetideas.table.client.filter.ColumnFilterListener;
...
public class ScrollTable extends ComplexPanel implements ResizableWidget,
   ...
   */
  protected Element dataWrapper;

  private ColumnFilterListener columnFilterListener = new ColumnFilterListener() {
    public void onFilterChanged(ColumnFilterInfo info) {
  ...
  /**
   * A map of columns that cannot be filtered.
   * 
   * key = the column index
   * 
   * value = true if the column is filterable, false of not
   */
  private Map<Integer, Boolean> unfilterableColumns = new HashMap<Integer, Boolean>();
  ...
  public boolean isColumnFilterable(int column) {
    Boolean filterable = unfilterableColumns.get(new Integer(column));
    if (filterable == null) {
      ...
      case Event.ONMOUSEUP:
        ...
        if (resizeWorker.isResizing()) {
          ...
          if (cellElem != null) {
            ...
            if (isColumnSortable(column) && isHeaderRowSortable(row)) {
  ...
  public void setColumnFilterable(int column, boolean filterable) {
    unfilterableColumns.put(new Integer(column), Boolean.valueOf(filterable));
  }
  ...
  protected void resizeTablesVerticallyNow() {
    ...
    DOM.setStyleAttribute(dataWrapper, "height", Math.max(0, totalHeight
