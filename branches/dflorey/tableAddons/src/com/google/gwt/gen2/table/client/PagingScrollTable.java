 ...
 */
public class PagingScrollTable<RowType> extends AbstractScrollTable implements HasTableDefinition<RowType>, HasPageCountChangeHandlers, HasPageLoadHandlers,
		HasPageChangeHandlers, HasPagingFailureHandlers {
	/**
	...
	private class VisibleRowsIterator implements Iterator<RowType> {
		 ...
		 * 
		 * @param rows
		 *            the iterator over row data
		 * @param firstRow
		 *            the first absolute row of the rows iterator
		 * @param firstVisibleRow
		 *            the first visible row in this grid
		 * @param lastVisibleRow
		 *            the last visible row in this grid
		 */
	...
	/**
	 * Default filter delay. Starts table filtering if filters did not change
	 * for 2 seconds
	 */
	...
	/**
	 * The number of rows per page. If the number of rows per page is equal to
	 * the number of rows, paging is disabled because only one page exists.
	 */
	...
	/**
	 * The values associated with each row. This is an optional list of data
	 * that ties the visible content in each row to an underlying object.
	 */
	 ...
	 *            the underlying table model
	 * @param dataTable
	 *            the table used to display data
	 * @param headerTable
	 *            the header table
	...
	public PagingScrollTable(TableModel<RowType> tableModel, FixedWidthGrid dataTable, FixedWidthFlexTable headerTable,
		...
		if (tableModel instanceof HasRowValueChangeHandlers) {
			((HasRowValueChangeHandlers<RowType>) tableModel).addRowValueChangeHandler(new RowValueChangeHandler<RowType>() {
				public void onRowValueChange(RowValueChangeEvent<RowType> event) {
					setRowValue(event.getRowIndex() - getAbsoluteFirstRowIndex(), event.getRowValue());
				}
			});
		}
		...
		if (dataTable.getColumnFilter() == null) {
			ColumnFilterer filterer = new ColumnFilterer() {
				...
				public void onFilterColumn(SortableGrid grid, ColumnFilterList filterList, final ColumnFilterCallback callback) {
					if (timer != null) {
						timer.cancel();
					}
					timer = new Timer() {
	...
	/**
	 * Wait for an amount of time before filtering the tables to avoid many
	 * filter request when user is entering filter data
	 * 
	...
	public int getMinimumColumnWidth(int column) {
		...
		return Math.max(FixedWidthGrid.MIN_COLUMN_WIDTH, colDef.getMinimumColumnWidth());
	}
	 ...
	 *            the page
	 * @param forced
	 *            reload the page even if it is already loaded
	 */
	public void gotoPage(int page, boolean forced) {
		...
		if (currentPage != oldPage || forced) {
			...
			lastRequest = createRequest(currentPage * pageSize, pageSize, dataTable.getColumnSortList(), dataTable.getColumnFilterList());
	...
	public boolean isColumnFilterable(int column) {
		return isFilteringEnabled() && getColumnDefinition(column) != null && getColumnDefinition(column).getColumnFilter() != null;
	}
	 ...
	 * 
	 * @param emptyTableWidget
	 *            the widget to display when the data table is empty
	 */
	 ...
	 * 
	 * By default, the page size is zero, which indicates that all rows should
	 ...
	 *            the column sort list
	 * @param columnFilterList
	...
	protected void refreshVisibleColumnDefinitions() {
		visibleColumns = new ArrayList<ColumnDefinition<RowType, ?>>(tableDefinition.getVisibleColumnDefinitions());
	}
	...
	/**
	 * Set a block of data. This method is used when responding to data
	 * requests.
	 * 
	 * This method takes an iterator of iterators, where each iterator
	 * represents one row of data starting with the first row.
	 * 
	 * @param firstRow
	 *            the row index that the rows iterator starts with
	 * @param rows
	 ...
	 * 
	 * @param visible
	 *            true to show the empty table widget
	 */
