...
public class CachedTableModel<RowType> extends MutableTableModel<RowType> {
  ...
  /**
   * The filter list included with the last request.
   */
  private ColumnFilterList lastFilterList = null;
  ...
  public void requestRows(Request request, Callback<RowType> callback) {
    // Clear the cache if the sort order or filter has changed
    ...
    ColumnFilterList filterList = request.getColumnFilterList();
    if (filterList == null) {
      if (lastFilterList != null) {
        clearCache();
        lastFilterList = null;
      }
    } else if (!filterList.equals(lastFilterList)) {
      clearCache();
      lastFilterList = filterList.copy();
    }
