...
public class TimingLog {
  ...
  private static Duration lastChecked;
  ...
  public static void logTiming(String message) {
    if (currentlyTiming) {
      if (Log.isLoggingSupported() && !Log.isLoggingMinimal()) {
        if (lastChecked == null) {
          Log.info(message + ": time " + time.elapsedMillis() + "ms", CATEGORY);
        } else {
          Log.info(message + ": time " + time + "ms of "
              + lastChecked.elapsedMillis() + "ms", CATEGORY);
        }
        lastChecked = new Duration();
      }
    }
  }
